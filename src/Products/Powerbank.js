...
const Powerbank = () => {
  ...
  const { state: { brand, powerbank, checked, checked1,checked2,checked3,checked4,checked5,checked6,checked7,checked8,searchQuary}, dispatch } = useContext(stateContext);
  ...
  const transformedProduct = () => {
    ...
    if(brand === 'gold' && checked){
      sortedProduct = sortedProduct.filter(e => e.sub.toLowerCase().includes(brand.toLowerCase()))
      return sortedProduct;
    }
    
    if(brand === 'blue' && checked1){
      sortedProduct = sortedProduct.filter(e => e.sub.toLowerCase().includes(brand.toLowerCase()))
      return sortedProduct
    }

    if(brand === 'beige' && checked2){
      sortedProduct = sortedProduct.filter(e => e.sub.toLowerCase().includes(brand.toLowerCase()))
      return sortedProduct
    }

    if(brand === 'green' && checked3){
      sortedProduct = sortedProduct.filter(e => e.sub.toLowerCase().includes(brand.toLowerCase()))
      return sortedProduct
    }
    ...
    {option}
                ...
                <div className='checked'>
                  ...
                  <input type="checkbox"
                    ...
                    onChange={() => dispatch({
                      type: 'FILTER_BRAND',
                      payload :{ name : 'gold', bool : true}
                    })} id="powerbank1" name="powerbank1" value="powerbank"></input>
                  <label for="powerbank1"> gold</label><br></br>
                  <input type="checkbox" 
                   checked={checked1}
                   onChange={() => dispatch({
                     type: 'FILTER_BRAND1',
                     payload :{ name : 'blue', bool : true}
                   })}
                   id="powerbank2" name="powerbank2" value="powerbank"></input>
                  <label for="powerbank2"> blue</label><br></br>
                  <input type="checkbox"
                  checked={checked2}
                    onChange={() => dispatch({
                      type: 'FILTER_BRAND2',
                      payload :{ name : 'beige', bool : true}
                    })}
                     id="powerbank3" name="powerbank3" value="powerbank"></input>
                  <label for="powerbank3"> beige</label><br></br>
                  <input type="checkbox"
                  checked={checked3}
                    onChange={() => dispatch({
                      type: 'FILTER_BRAND3',
                      payload :{ name : 'green', bool : true}
                    })} id="powerbank4" name="powerbank4" value="powerbank"></input>
                  <label for="powerbank4"> green</label><br></br>
                </div>
                <br></br>
                <hr></hr>
